...
rkappa = 0.9;   	% Time derivative preconditioning constant
...
u(1,:,3) = 0;                                       %y-velocity
%Right Boundary--------------------------------------------
u(imax,:,1) = 2.*u(imax-1,:,1) - u(imax-2,:,1);     %Pressure
u(imax,:,2) = 0;                                    %x-velocity
u(imax,:,3) = 0;                                    %y-velocity
%Bottom Boundary-------------------------------------------
u(:,1,1) = 2.*u(:,2,1) - u(:,3,1);     %Pressure
u(:,1,2) = 0;                                    %x-velocity
u(:,1,3) = 0;                                    %y-velocity
%Top Boundary----------------------------------------------
u(:,jmax,1) = 2.*u(:,jmax-1,1) - u(:,jmax-2,1);                  %Pressure
u(:,jmax,2) = uinf;                                    %x-velocity
u(:,jmax,3) = 0;                                       %y-velocity
...
function [dtmin] = compute_time_step(dtmin)
    ...
    % !************************************************************** */
    %define consts
    dtvisc = (dx*dy)/(four*(rmu/rho)); %diffusive time step                   %dummy value for max time step
    %loop for minimum timestep
    for i = 2:imax-1
        for j = 2:jmax-1
            beta2 = max(u(i,j,2)*u(i,j,2)+u(i,j,3)*u(i,j,3),rkappa*vel2ref);     %calculate beta^2
            lambda_x = half*(abs(u(i,j,2))+sqrt(u(i,j,2)*u(i,j,2)+four*beta2));    %find x eigenvalue
            lambda_y = half*(abs(u(i,j,3))+sqrt(u(i,j,3)*u(i,j,3)+four*beta2));    %find y eigenvalue
            lambda_max = max(lambda_x,lambda_y);                         %find max eigenvalue
            dtconv = min(dx,dy)/abs(lambda_max);                               %convective time step
            dt(i,j) = cfl*min(dtconv,dtvisc);                                 %define minimum timestep at point
            dtmin = min(dt(i,j),dtmin);                                   %calculate max time step for global time stepping
        end
    end
end
%************************************************************************
function Compute_Artificial_Viscosity(~)
    ...
    lambda_x = 0;
    lambda_y = 0;
    %Find maximum lambdas in domain
    for i = 1:imax
        for j = 1:jmax
            beta2(i,j) = max(u(i,j,2)*u(i,j,2)+u(i,j,3)*u(i,j,3),rkappa*vel2ref);     %calculate beta^2        
            lambda_x = max(lambda_x,half*(abs(u(i,j,2))+sqrt(u(i,j,2)*u(i,j,2)+four*beta2(i,j))));    %find x eigenvalue
            lambda_y = max(lambda_y,half*(abs(u(i,j,3))+sqrt(u(i,j,3)*u(i,j,3)+four*beta2(i,j))));    %find y eigenvalue
        end
    end
    ...
    for i = 3:imax-2
        for j = 3:jmax-2
            ...
            artviscx(i,j) = -((abs(lambda_x)*Cx)/beta2(i,j))*d4pdx4;
            artviscy(i,j) = -((abs(lambda_y)*Cy)/beta2(i,j))*d4pdy4;
        end
    end
    %Set artvisc for "boundaries"
    %i=2
    artviscx(2,:) = artviscx(3,:);
    artviscy(2,:) = artviscy(3,:);
    %i = imax-1
    artviscx(imax-1,:) = artviscx(imax-2,:);
    artviscy(imax-1,:) = artviscy(imax-2,:);
    %j = 2
    artviscx(:,2) = artviscx(:,3);
    artviscy(:,2) = artviscy(:,3);
    %j = jmax-1
    artviscx(:,jmax-1) = artviscx(:,jmax-2);
    artviscy(:,jmax-1) = artviscy(:,jmax-2);

    %Set artvisc for corners
    %i=2,j=2
    artviscx(2,2) = 0.5*(artviscx(3,2)+artviscx(2,3));
    artviscy(2,2) = 0.5*(artviscy(3,2)+artviscy(2,3));
    %i=2,j=jmax-1
    artviscx(2,jmax-1) = 0.5*(artviscx(2,jmax-2)+artviscx(3,jmax-1));
    artviscy(2,jmax-1) = 0.5*(artviscy(2,jmax-2)+artviscy(3,jmax-1));
    %i=imax-1,j=2
    artviscx(imax-1,2) = 0.5*(artviscx(imax-2,2)+artviscx(imax-1,3));
    artviscy(imax-1,2) = 0.5*(artviscy(imax-2,2)+artviscy(imax-1,3));
    %i=imax-1,j=jmax-1
    artviscx(imax-1,jmax-1) = 0.5*(artviscx(imax-2,jmax-1)+artviscx(imax-1,jmax-2));
    artviscy(imax-1,jmax-1) = 0.5*(artviscy(imax-2,jmax-1)+artviscy(imax-1,jmax-2));
end
%************************************************************************
...
function point_Jacobi(~)
    ...
    % !************ADD CODING HERE FOR INTRO CFD STUDENTS************ */
    % !************************************************************** */
    uold = u; %keep old values for iterative convergence
    ...
    for i = 2:imax-1
        for j = 2:jmax-1
            ...
            % x momentum eqation 
            u(i,j,2)= uold(i,j,2) - (dt(i,j)*rhoinv)*(rho*u(i,j,2)*dudx + rho*u(i,j,3)*dudy + dpdx - rmu*d2udx2 - rmu*d2udy2 - s(i,j,2));
            % y momentum equation
            u(i,j,3)= uold(i,j,3) - (dt(i,j)*rhoinv)*(rho*u(i,j,2)*dvdx + rho*u(i,j,3)*dvdy + dpdy - rmu*d2vdx2 - rmu*d2vdy2 - s(i,j,3));
        end
    end
...
function [res, resinit, conv] = check_iterative_convergence...
    ...
    % !************ADD CODING HERE FOR INTRO CFD STUDENTS************ */
    % !************************************************************** */
    if n == 1 %check to see if we are on the first iteration
        %Calculate initial
        ...
        res(2) = sqrt(sum(abs((u(:,:,2)-uold(:,:,2))./dt(:,:)).^2,'all')/(imax*jmax));%L2 norm of x-mtm
        res(3) = sqrt(sum(abs((u(:,:,3)-uold(:,:,3))./dt(:,:)).^2,'all')/(imax*jmax));%L2 norm of y-mtm
    end
...
function Discretization_Error_Norms(rL1norm, rL2norm, rLinfnorm)
    ...
    if imms==1 %Compute DE Norms with MMS
    % !************************************************************** */
    % !************ADD CODING HERE FOR INTRO CFD STUDENTS************ */
    % !************************************************************** */
        %Find the L_inf norm
        rLinfnorm(1) = max(max(abs(u(:,:,1))));
        rLinfnorm(2) = max(max(abs(u(:,:,2))));
        rLinfnorm(3) = max(max(abs(u(:,:,3))));
        %Find the L_1 norm
        rL1norm(1) = sum(abs(u(:,:,1)),'all')/(imax*jmax);
        rL1norm(2) = sum(abs(u(:,:,2)),'all')/(imax*jmax);
        rL1norm(3) = sum(abs(u(:,:,3)),'all')/(imax*jmax);
        %Find the L_2 norm
        rL2norm(1) = sqrt(sum(abs(u(:,:,1)).^2,'all')/(imax*jmax));
        rL2norm(2) = sqrt(sum(abs(u(:,:,2)).^2,'all')/(imax*jmax));
        rL2norm(3) = sqrt(sum(abs(u(:,:,3)).^2,'all')/(imax*jmax));
    else %Compute DE norms without MMS
        %Find the L_inf norm
        rLinfnorm(1) = max(max(abs(u(:,:,1))));
        rLinfnorm(2) = max(max(abs(u(:,:,2))));
        rLinfnorm(3) = max(max(abs(u(:,:,3))));
